...
func (g *ModelGenerator) StructName() string {
	return g.Model.Settings().Name
}
...
func (g *ModelGenerator) AddJSONMethod() {
	...
	g.File.Func().Params(
		jen.Id("model").Op("*").Id(g.StructName()),
	).Id("PrintJSON").Params(
		// No function args
	).Params(
		// Returns nothing
...
func (g *ModelGenerator) Generate() {
	...
	g.File.Comment("// " + g.StructName() + " is a database model which represents a single row from the \n" +
		"// "+g.Model.Settings().DBTable+" database table")
	g.File.Type().Id(g.StructName()).Struct(fields...)
